...
class DualPrompt(_Trainer):
    ...
    def online_evaluate(self, test_loader, task_id=None, end=False):
        ...
        with torch.no_grad():
            for i, data in enumerate(test_loader):
                ...
                if self.only_test:
                    res = self.model.forward_feature(x)
                    # print("cat feature--------------")
                else:
                    res = self.model(x)
                if isinstance(res, tuple):
    ...
    #     idx = torch.randperm(self.model_without_ddp.final_features.shape[0])
    #     print(self.labels.size())
    #     print(self.model_without_ddp.final_features.shape)
    #     labels = self.labels[idx[:10000]]

    #     self.model_without_ddp.final_features = torch.cat([self.model_without_ddp.final_features[idx[:10000]], self.model_without_ddp.keys], dim=0)
    #     self.model_without_ddp.final_features = F.normalize(self.model_without_ddp.final_features, dim=1)

    #     tsne = TSNE(n_components=2, random_state=0)
    #     X_2d = tsne.fit_transform(self.model_without_ddp.final_features.detach().cpu().numpy())
