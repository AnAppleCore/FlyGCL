#!/bin/bash
# --------------------------------------------------------------
# FlyAdapter MISA-style adapter DDP pretraining launcher on ImageNet-1k.
#
...
#   IMAGENET_ROOT=/data/datasets/ImageNet EPOCHS=32 bash scripts/run_flyadapter_misa_pretrain_ddp.sh
# Inspect commands without launching screen jobs:
#   DRY_RUN=1 bash scripts/run_flyadapter_misa_pretrain_ddp.sh
# --------------------------------------------------------------
set -euo pipefail

PROJECT_DIR=${PROJECT_DIR:-/home/hongwei/FlyGCL}
PYTHON=${PYTHON:-/home/hongwei/miniconda3/envs/DGIL/bin/python}
IMAGENET_ROOT=${IMAGENET_ROOT:-/data/datasets/ImageNet}
DRY_RUN=${DRY_RUN:-0}

LOG_DIR=${LOG_DIR:-${PROJECT_DIR}/results/logs/flyadapter_misa_pretrain_ddp}
CKPT_DIR=${CKPT_DIR:-${PROJECT_DIR}/checkpoints/FlyAdapter_MISA_Pretrain_Adapter}
...
SUB_PORT=${SUB_PORT:-29771}
ADD_PORT=${ADD_PORT:-29772}
RUN_SUB=${RUN_SUB:-1}
RUN_ADD=${RUN_ADD:-1}
...
run_one() {
  ...
  local port=$3
  local extra_flag=$4
  local screen_name="flyadapter_misa_${name}"
  local exp_name="flyadapter_misa_${name}_ddp_bs${GLOBAL_BATCH_SIZE}_ep${EPOCHS}_seed${SEED}_adim${FLY_ADAPTER_DOWN_DIM}_layers${FLY_ADAPTER_LAYERS}"
  local log_file="${LOG_DIR}/${exp_name}.log"
  local ckpt_dir="${CKPT_DIR}/${exp_name}"
  local ckpt_file="${ckpt_dir}/flyadapter_misa_adapter_${name}_ddp_bs${GLOBAL_BATCH_SIZE}_ep${EPOCHS}_seed${SEED}.pt"
  local train_cmd

  mkdir -p "${ckpt_dir}"

  train_cmd="set -euo pipefail; cd '${PROJECT_DIR}'; CUDA_VISIBLE_DEVICES='${gpus}' '${PYTHON}' -m torch.distributed.run --nproc_per_node ${NPROC_PER_NODE} --master_port ${port} pretrain_flyprompt_misa.py --expert_type adapter --imagenet_root '${IMAGENET_ROOT}' --backbone '${BACKBONE}' --num_init_classes ${NUM_INIT_CLASSES} --num_id_classes ${NUM_ID_CLASSES} --ood_active_classes ${OOD_ACTIVE_CLASSES} --fly_adapter_down_dim ${FLY_ADAPTER_DOWN_DIM} --fly_adapter_layers ${FLY_ADAPTER_LAYERS} --image_size 224 --batch_size ${PER_RANK_BATCH_SIZE} --ood_batch_size ${PER_RANK_OOD_BATCH_SIZE} --epochs ${EPOCHS} --lr ${LR} --rho ${RHO} --weight_decay 0.0 --num_workers ${NUM_WORKERS} --seed ${SEED} --device cuda --log_interval 50 --save_every_epoch ${extra_flag} --save_path '${ckpt_file}' > '${log_file}' 2>&1"
